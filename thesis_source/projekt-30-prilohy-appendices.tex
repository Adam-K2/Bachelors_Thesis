...
\chapter{Obsah pamäťového média}

Na priloženom pamäťovom médiu sa nachádza nasledujúca stromová štruktúra:

\dirtree{%
.1 /.
.2 application.\DTcomment{Zdrojové súbory aplikácie}.
.3 app.py\DTcomment{Skript výslednej aplikácie}.
.2 autocrop.\DTcomment{Autocrop knižnica na orezávanie snímkov podľa tváre}.
.2 dataset.\DTcomment{Položka so všetkými snímkami použitých v~práci}.
.2 face\_rec\_experiments.\DTcomment{Experimenty s algoritmami pre rozpoznanie tváre}.
.2 metacentrum.\DTcomment{Súbory pre prácu na Metacentre}.
.2 neural\_env.\DTcomment{Prostredie obsahujúce python skripty}.
.3 meta-graphs.ipynb.\DTcomment{Skript pre tvorbu grafov z~výstupu Metacentra}.
.3 neural\_start.ipynb.\DTcomment{Skript pre siete s~jedným vstupom}.
.3 siamese.ipynb.\DTcomment{Skript pre siamské siete}.
.2 thesis\_source.\DTcomment{Zdrojové súbory \LaTeX}.
}
